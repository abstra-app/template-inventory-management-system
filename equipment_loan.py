...
free_equipments = [dict(t)
                   for t in {tuple(d.items()) for d in free_equipments}]

if free_equipments:
    free_equipments_labels = [{"label": e["description"], "value": e["id"]}
                              for e in free_equipments]
else:
    free_equipments_labels = [
        {"label": "No equipment available", "value": None}]
...
if equipment_page.get("equipments"):
    loaned_equipment_team_member = at.select(
        "team", where={"id": equipment_page.get("team_member")})[0]

    equipment_data = [equipment for equipment in free_equipments if equipment["id"]
